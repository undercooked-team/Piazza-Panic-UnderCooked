...
abstract class ScenarioLoadLogic extends GameLogic {
  ...
  /**
   * Constructor for the {@link ScenarioLoadLogic}.
   *
   ...
   */
  public ScenarioLoadLogic(GameScreen game, TextureManager textureManager,
  ...
  protected void loadScenario(String scenarioAsset) {
    ...
    for (JsonValue interaction : scenarioData.get("interactions")) {
      interactions.loadInteractionAsset(interaction.asString(),
                                        stationController, audioManager,
                                        items);
    }
  ...
  protected void loadRequests(JsonValue requestData) {
    ...
    for (JsonValue request : requestData) {
      ...
      if (request.isString()) {
        ...
        thisReqData = FileControl.loadJsonAsset(request.asString(), "requests");
        // If it's null, skip
        if (thisReqData == null) {
          continue;
        }
        // Otherwise, make sure it's formatted correctly
        JsonFormat.formatJson(thisReqData, requestFormat);
      ...
      if (items.addItemAsset(thisReqData.getString("item_id")) == null) {
        continue;
      }
      // If it was successfully added, then add it to the array
      Request newRequest = new Request(thisReqData.getString("item_id"));
      newRequest.setValue(thisReqData.getInt("value"));
      ...
      JsonValue instructions = thisReqData.get("instructions");
      // Loop through the instructions
      for (JsonValue instruction : instructions) {
        // Add them to the Request's instructions
        Instruction newInstruction = newRequest.addInstruction(
                instruction.getString("texture_path"),
      ...
      newRequest.setReputationThreat(thisReqData.getInt("reputation_threat"));

      // Check the time value of the request
      float time = thisReqData.getFloat("time");
      if (time < 0) {
        // If the time is invalid, default to -1
        time = -1;
        // If it's < 0, then check for time_min and time_max
        float timeMin = thisReqData.getFloat("time_min");
        float timeMax = thisReqData.getFloat("time_max");

...
import com.undercooked.game.util.json.JsonFloat;
import com.undercooked.game.util.json.JsonFormat;
import com.undercooked.game.util.json.JsonInt;
import com.undercooked.game.util.json.JsonObject;
import com.undercooked.game.util.json.JsonOr;
import com.undercooked.game.util.json.JsonString;
import com.undercooked.game.util.json.JsonType;
...
public final class DefaultJson {
   ...
   * {@link JsonValue}.
   *
   * @return {@link JsonObject} : The Json formatting to use in
   *                              {@link JsonFormat#formatJson(JsonValue, JsonObject)}.
   */
  public static JsonObject leaderboardFormat() {
    ...
    // Time in scenario, Number of Customers served in endless.
    highscore.addValue(new JsonFloat("score", -1F));
    // The date & time it was recorded
    highscore.addValue(new JsonString("date", Constants.UNKNOWN_DATE, false));
    final JsonArray highscores = new JsonArray("scores", highscore);

    // Scenario scores
    final JsonObject score = new JsonObject("scores");
  ...
  public static JsonObject stationFormat() {
    ...
    // How many x tiles it takes up
    root.addValue(new JsonInt("width", 1));
    // How many y tiles it takes up
    root.addValue(new JsonInt("height", 1));

    // The width of the collision box in pixels
    root.addValue(new JsonFloat("collision_width", -1F));
    // The height of the collision box in pixels
    root.addValue(new JsonFloat("collision_height", -1F));

    // The z offset of the collision box in pixels
    root.addValue(new JsonFloat("collision_offset_x", 0F));
    // The y offset of the collision box in pixels
    root.addValue(new JsonFloat("collision_offset_y", 0F));
    ...
    // How many items the station holds.
    root.addValue(new JsonInt("holds", 0));

    // The default price of the station, if not set on the map
    root.addValue(new JsonInt("price", 0));
    return root;
  }
  ...
  public static JsonObject scenarioFormat() {
    ...
    final JsonObject cook = new JsonObject();
    ...
    //// Power Ups
    final JsonObject powerUps = new JsonObject("power_ups");
  ...
  public static JsonVal<?> requestFormat(boolean asOr) {
    ...
    final JsonObject asObject = new JsonObject();

    // The recipe information
    asObject.addValue(new JsonString("item_id", null));

    // Value of the request. If < 0, then uses item's default value.
    asObject.addValue(new JsonInt("value", -1));

    // The definite time that the request must be provided by.
    // If the above is not set, then the below two are checked
    asObject.addValue(new JsonFloat("time", -1F));

    // The lowest time that the request must be provided by.
    asObject.addValue(new JsonFloat("time_min", -1F));

    // The highest time that the request must be provided by.
    asObject.addValue(new JsonFloat("time_max", -1F));

    // The number of times that the request can be made.
    asObject.addValue(new JsonInt("number", 1));

    // How much reputation is taken if the player fails to complete the request
    asObject.addValue(new JsonInt("reputation_threat", 1));

    // The directions for how to make the item
    final JsonObject recipe = new JsonObject("instruction");
  ...
  public static JsonObject interactionFormat() {
    ...
    // The station ID for the interaction
    root.addValue(new JsonString("station_id", null));

    // An array of item IDs. The exact same number are needed for the interaction to take place,
    // the interaction only starting once a valid interaction is found.
    root.addValue(new JsonArray("items", new JsonString(null, null)));

    // A step in the interaction
    final JsonObject interactionStep = new JsonObject("intStep");

    // The type
    interactionStep.addValue(new JsonString("type", null));
    /*
        Examples of types:
        ...
        - Set : Sets the items on the station to the list provided.
                Ignores time and moves to next step.
        - Input : Requires an input of id value to move to the next step.
        - Input timed : Requires an input within a time limit.
     */

    // Can be anything, relying on the type.
    interactionStep.addValue(new JsonString("value", null));

    // Step path to take if the interaction is a success
    interactionStep.addValue(new JsonArray("success", interactionStep));

    // Step path to take if the interaction is a failure
    // (won't be needed for all interaction types)
    interactionStep.addValue(new JsonArray("failure", interactionStep));

    // The time (in seconds) that this step takes.
    interactionStep.addValue(new JsonFloat("time", -1F));
    // The sound this step makes.
    interactionStep.addValue(new JsonString("sound", null));
    ...
    '''
    {
        ...
        *Further interactions that happen regardless of success and failure*

        ]
    }
    '''

    Visually, this could be shown as one main bar, and a faded bar above.
    If you fail, the main bar disappears, and the faded bar replaces it.

...
public class AudioManager {
   ...
   *         or the {@link Constants#DEFAULT_MUSIC} file if not.
   */
   ...
   *         file to be loaded by the {@link AssetManager},
  ...
  public void unloadMusic(String path) {
    ...
    if (assetManager.isLoaded(path)) {
      ...
      for (VolumeGroup volGroup : musicVolumes.values()) {
        volGroup.removePath(path);
      }
  ...
  public void updateMusicVolumes(String musicVolumeGroup) {
    ...
    VolumeGroup volGroup = musicVolumes.get(musicVolumeGroup);
    for (String path : volGroup.paths) {
      // Loop through the paths, get the music and update
      // their volumes.
      try {
        Music thisMusic = assetManager.get(path);
9        thisMusic.setVolume(volGroup.volume);
      } catch (GdxRuntimeException e) {
        e.printStackTrace();
      }
    }
   ...
   *
   * @param audioGroup  A {@link String} of the {@code audioGroup}'s id.
   * @param forgetGroup {@code boolean} : If the volume group should be deleted
   *                    afterwards or not.
   */
  ...
  /**
   * Disposes of all music and their volumes.
   */
  public void dispose() {
    // Go through all the music and unload them.
    for (VolumeGroup volMusicGroup : musicVolumes.values()) {
      for (String path : volMusicGroup.paths) {
        assetManager.unload(path);
      }
    }
    ...
    // Go through all the sounds and unload them.
    for (VolumeGroup volSoundGroup : soundVolumes.values()) {
      for (String path : volSoundGroup.paths) {
        assetManager.unload(path);
      }
    }
    // Then clear the volume map

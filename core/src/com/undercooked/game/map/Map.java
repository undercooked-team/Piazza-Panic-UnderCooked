...
public class Map {
   ...
   * @param floorTile {@link String} : The asset path to the floor tile
   *                                   {@link com.badlogic.gdx.graphics.Texture} to use.
   *                                   If {@code null}, no floor
   *                                   {@link com.badlogic.gdx.graphics.Texture}.
   * @return {@link Array}&lt;{@link Entity}&gt; : An {@link Array} of all {@link MapEntity}
   *                                               that were removed from the {@link Map}.
   */
   ...
   *                                      If {@code null}, no floor
   *                                      {@link com.badlogic.gdx.graphics.Texture}.
   * @param hasCollision {@code boolean} : Whether the {@link MapCell}s that the {@link MapEntity}
   *                                       take up will or will not have collision.
   * @return {@link Array}&lt;{@link Entity}&gt; : An {@link Array} of all {@link MapEntity}
   *                                               that were removed from the {@link Map}.
   */
  public Array<Entity> addFullMapEntity(MapEntity entity, int x, int y,
                                        String floorTile, boolean hasCollision) {
      ...
      for (int j = y + entity.getCellHeight() - 1; j >= y; j--) {
        ...
        if (validCellFull(i, j)) {
          ...
          if (cellToReplace.getMapEntity() != null) {
            ...
            if (!removedEntities.contains(entityToRemove, true)) {
              removedEntities.add(entityToRemove);
            }
          }
        ...
        if (cellBelow.getMapEntity() == null) {
          ...
          if (cellBelow.isBase()) {
            ...
            if (entity.basePath != null) {
              ...
              // Add entity to removed entities, if it's not there already
              if (!removedEntities.contains(cellBelow.mapEntity, true)) {
                removedEntities.add(cellBelow.mapEntity);
              }
            ...
            // Note that this has a problem when a station with collision has a station
            // placed above it that does not have a base, as it will remove the collision
   ...
   * @param hasCollision {@code boolean} : Whether the {@link MapCell}s that the {@link MapEntity}
   *                                       take up will or will not have collision.
   * @return {@link Array}&lt;{@link Entity}&gt; : An {@link Array} of all {@link MapEntity} that
   *                                               were removed from the {@link Map}.
   */
  public Array<Entity> addMapEntity(MapEntity entity, int x, int y,
                                    String floorTile, boolean hasCollision) {
    ...
    if (!(validCell(x, y) && validCell(x + entity.getCellWidth() - 1,
            y + entity.getCellHeight() - 1))) {
   ...
   *
   * @return {@link Array}&lt;{@link MapCell}&gt; : An {@link Array} of the open {@link MapCell}s.
   */
   ...
   * @param height      {@code int} : The height
   * @param mapCellType {@code int} : The {@link MapCellType} to ignore.
   * @return {@link Array}&lt;{@link MapCell}&gt; : An {@link Array} of all {@link MapCell}s
   *                                                in the range provided that match the
   *                                                {@code mapCellType} condition.
   */
  public Array<MapCell> openCellsRangeFull(int x, int y,
  ...
  public MapCell getCollision(Rectangle collision, boolean returnClosest, MapCellType mapCellType) {
    ...
    int cellMaxX = MapManager.posToGridFloor(collision.x + collision.width);

    int cellY = MapManager.posToGridFloor(collision.y);
    int cellMaxY = MapManager.posToGridFloor(collision.y + collision.height);
    ...
    for (int x = cellX; x <= cellMaxX; x++) {
      for (int y = cellY; y <= cellMaxY; y++) {
    ...
    if (returnClosest) {
      ...
      double closestDist = MathUtil.distanceBetweenRectangles(collision,
              closestCell.getCollision());
      ...
      for (int i = 1; i < validCells.size; i++) {
        ...
        double currentDist = MathUtil.distanceBetweenRectangles(collision,
                currentCell.getCollision());
  ...
  /**
   * Returns the width of the full map.
   *
  ...
  /**
   * Returns the height of the full map.
   *
  ...
  /**
   * Returns the offset of the play area on the full map for the
   * x-axis.
   *
  ...
  /**
   * Returns all of the {@link MapEntity}s that are present on the map.
   *
   * @return {@link Array}&lt;{@link MapEntity}&gt; : All of the {@link MapEntity}s from the
   *                                                  {@link MapCell}s.
   */
